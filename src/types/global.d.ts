...
interface AiPlayerAPI {
  ...
  mediaTools?: {
    planEdit: (input: { instruction: string; sourcePath: string }) => Promise<{ matched: boolean; decision?: { schemaVersion: 1; kind: 'media.trim' | 'media.remove-segment'; timeline: { startSeconds: number; endSeconds: number; durationSeconds?: number; removedDurationSeconds?: number }; output: { overwrite: false; suffix: string } }; error?: string }>
    ...
    trim: (input: { instruction: string; sourcePath: string; requestId: string; workspaceTaskId?: string }) => Promise<{ success: boolean; matched?: boolean; requestId?: string; cancelled?: boolean; outputPath?: string; outputs?: string[]; durationSeconds?: number; expectedDurationSeconds?: number; timelineReceipt?: Array<{ operation: string; sourceRange: string; outputRange: string }>; projectCapsule?: { schemaVersion: 1; projectId: string; versionId: string; currentPath: string; cursor: number; versionCount: number; canUndo: boolean; canRedo: boolean }; summary?: string; error?: string }>
