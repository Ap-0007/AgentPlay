...
const EDIT_WITHOUT_SOURCE_INTENT = /(?:剪一下|剪辑|裁剪|剪出|删除视频|删掉视频|防抖|稳定画面|自动修复曝光|修复偏色|多轨混音|添加环境声|添加音效|对白闪避|分段音量|降噪|去直流|响度匹配|静音修复|分离人声|提取伴奏|(?:删除|删掉|保留|留下|拼接|重排)[\s\S]*(?:秒|分钟|片段)|(?:16\s*[:：]\s*9[\s\S]*9\s*[:：]\s*16|横屏[\s\S]*竖屏)[\s\S]*(?:跟踪|追踪|主体|方形))/
