...
export default function useMediaCreativeTasks(options: MediaCreativeTaskOptions) {
  ...
  const runPersonalEditSkillCommand = async (text: string): Promise<boolean> => {
    if (!text || !window.aiPlayer?.personalEditSkills?.plan || !window.aiPlayer?.personalEditSkills?.execute) return false
    const planned = await window.aiPlayer.personalEditSkills.plan({ instruction: text })
    if (!planned?.matched) return false
    addMessage('user', text); setInputText('')
    const result = await window.aiPlayer.personalEditSkills.execute({ instruction: text })
    if (!result?.success) { addMessage('agent', `[错误] ${result?.error || '个人编辑Skill操作失败'}`); return true }
    addMessage('agent', result.summary || '个人编辑Skill已更新')
    return true
  }
  ...
  const runTrimTask = async (text: string, override?: TrimInput): Promise<boolean> => {
    ...
    const personalSkill = frozenDecision?.personalEditSkill
    executionTaskIdRef.current = startTask({
      kind: 'media', label: `${actionLabel}${personalSkill ? ` · ${personalSkill.name}` : ''}`, phase: 'running',
    ...
    try {
      ...
      addMessage('agent', `${personalSkill ? `已应用个人编辑Skill“${personalSkill.name}”（${personalSkill.fieldsApplied.join('、')}）。\n` : ''}${summary}${timeline ? `\n时间线：\n${timeline}` : ''}${projectHint}\n成果：${completedOutputs.join('\n')}`)
