...
type TrimInput = {
  ...
  startSeconds: number
  endSeconds: number
  operation?: 'trim' | 'remove' | 'concat' | 'music' | 'audio-mix' | 'audio-repair' | 'rhythm' | 'effects' | 'reframe' | 'repair' | 'subtitle' | 'shift' | 'mux' | 'translate' | 'cue-edit'
...
export default function useMediaCreativeTasks(options: MediaCreativeTaskOptions) {
  ...
  const runTrimTask = async (text: string, override?: TrimInput): Promise<boolean> => {
    ...
    let operation: 'trim' | 'remove' | 'concat' | 'music' | 'audio-mix' | 'audio-repair' | 'rhythm' | 'effects' | 'reframe' | 'repair' | 'subtitle' | 'shift' | 'mux' | 'translate' | 'cue-edit' = override?.operation || 'trim'
    ...
    if (!override) {
      try {
        ...
        if (!plan?.matched || !decision || !['media.trim', 'media.remove-segment', 'media.concat-segments', 'media.add-music', 'media.mix-audio', 'media.repair-audio', 'media.rhythm-edit', 'media.visual-effects', 'media.smart-reframe', 'media.visual-repair', 'media.concat-sources', 'media.burn-subtitles', 'media.shift-subtitles', 'media.mux-subtitles', 'media.translate-subtitles', 'media.edit-subtitle-cues'].includes(decision.kind)) {
          pendingEditClarificationRef.current = null
          return false
        }
        pendingEditClarificationRef.current = null
        ...
        operation = decision.kind === 'media.rhythm-edit' ? 'rhythm' : decision.kind === 'media.repair-audio' ? 'audio-repair' : decision.kind === 'media.mix-audio' ? 'audio-mix' : decision.kind === 'media.add-music' ? 'music' : decision.kind === 'media.visual-repair' ? 'repair' : decision.kind === 'media.smart-reframe' ? 'reframe' : decision.kind === 'media.visual-effects' ? 'effects' : decision.kind === 'media.burn-subtitles' ? 'subtitle' : decision.kind === 'media.mux-subtitles' ? 'mux' : decision.kind === 'media.translate-subtitles' ? 'translate' : decision.kind === 'media.edit-subtitle-cues' ? 'cue-edit' : decision.kind === 'media.shift-subtitles' ? 'shift' : decision.kind === 'media.remove-segment' ? 'remove' : decision.kind === 'media.concat-segments' || decision.kind === 'media.concat-sources' ? 'concat' : 'trim'
    ...
    if (retry.kind === 'trim' && retry.sourcePath && retry.instruction) {
      const planAndRetry = async () => {
        ...
        void runTrimTask(retry.instruction || '', {
          ...
          startSeconds: Number(decision.timeline?.startSeconds || decision.timeline?.segments?.[0]?.sourceStartSeconds || 0),
          endSeconds: Number(decision.timeline?.endSeconds || decision.timeline?.segments?.at(-1)?.sourceEndSeconds || 0),
          operation: decision.kind === 'media.rhythm-edit' ? 'rhythm' : decision.kind === 'media.repair-audio' ? 'audio-repair' : decision.kind === 'media.mix-audio' ? 'audio-mix' : decision.kind === 'media.add-music' ? 'music' : decision.kind === 'media.visual-effects' ? 'effects' : decision.kind === 'media.burn-subtitles' ? 'subtitle' : decision.kind === 'media.mux-subtitles' ? 'mux' : decision.kind === 'media.translate-subtitles' ? 'translate' : decision.kind === 'media.edit-subtitle-cues' ? 'cue-edit' : decision.kind === 'media.shift-subtitles' ? 'shift' : decision.kind === 'media.remove-segment' ? 'remove' : decision.kind === 'media.concat-segments' || decision.kind === 'media.concat-sources' ? 'concat' : 'trim',
          segments: (decision.timeline?.segments || []).map((segment) => ({ startSeconds: segment.sourceStartSeconds, endSeconds: segment.sourceEndSeconds })),
          decision
        })
      }
