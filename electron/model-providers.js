...
const RAW_PROVIDERS = [
  ...
  { id: 'codex-chatgpt', name: 'ChatGPT 订阅（经 Codex CLI）', region: '本地', protocol: 'cli', baseUrl: '', models: ['gpt-5.5', 'gpt-5.6-sol'], requiresKey: false, modelHint: '复用本机 Codex CLI 的 ChatGPT 订阅登录态（官方 CLI 自管 OAuth），只读子进程调用，不产生 API 费用。' },
  { id: 'claude-code', name: 'Claude 订阅（经 Claude Code）', region: '本地', protocol: 'cli', baseUrl: '', models: ['claude-sonnet-4.5', 'claude-opus-4.1'], requiresKey: false, modelHint: '复用本机 Claude Code 的 Claude 订阅登录态（官方 CLI 自管 OAuth），只读子进程调用，不产生 API 费用。' },
...
function validateProviderUrl(input) {
  ...
  // 订阅账号后端（Codex CLI / Claude Code）经子进程调用，不走网络栈，无 URL 可验
  if (config.protocol === 'cli') return { origin: 'cli://local' }
  let parsed
  try {
