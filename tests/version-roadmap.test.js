...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*80%（20\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*80%（20\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 25, 'five reusable foundations plus twenty completed features')
  assert.equal(checkboxCount(content, false), 5)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 0)
  assert.equal((content.match(/（未实现）/g) || []).length, 5)
  ...
  assert.match(content, /#### A\. 项目胶囊与持续记忆（4\/5 闭环，4\/5 有代码）/)
  assert.match(content, /#### B\. 任意文件与链接的统一理解（4\/5 闭环，4\/5 有代码）/)
  assert.match(content, /审计基线：`b084fb1`/)
  assert.match(content, /相对 `master` 有 35 个提交、96 个文件发生变化、12482 行新增/)
  assert.match(content, /128 个自动测试文件、30 个安装态 smoke 脚本/)
  assert.match(content, /752 tests、750 pass、0 fail、2 个外网条件 skip/)
  assert.match(content, /下一阶段没有“待收口”尾巴/)
  assert.match(content, /余下6项保持“未实现”/)
