...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*48%（12\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*76%（19\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 17, 'five reusable foundations plus twelve completed features')
  assert.equal(checkboxCount(content, false), 13)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 7)
  ...
  assert.match(content, /审计基线：`0ef459a`/)
  assert.match(content, /相对 `master` 有 31 个提交、88 个文件发生变化、11816 行新增/)
  assert.match(content, /125 个自动测试文件、29 个安装态 smoke 脚本/)
  assert.match(content, /742 tests、739 pass、0 fail、3 个外网条件 skip/)
  assert.match(content, /下一阶段只收口上述 7 项“已实现待收口”，不启动 6 项“未实现”的新功能开发/)
