...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*36%（9\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*76%（19\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 14, 'five reusable foundations plus nine completed features')
  assert.equal(checkboxCount(content, false), 16)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 10)
  assert.equal((content.match(/（未实现）/g) || []).length, 6)
  assert.match(content, /#### C\. 按成果自动编排工作流（3\/5 闭环，4\/5 有代码）/)
  assert.match(content, /#### D\. 自然语言影音编辑 Agent v1（5\/5 闭环，5\/5 有代码）/)
  assert.match(content, /#### E\. 后台完成与主动交付（1\/5 闭环，3\/5 有代码）/)
  assert.match(content, /审计基线：`a7656e7`/)
  assert.match(content, /相对 `master` 有 27 个提交、73 个文件发生变化、10743 行新增/)
  assert.match(content, /124 个自动测试文件、27 个安装态 smoke 脚本/)
  assert.match(content, /732 tests、728 pass、0 fail、4 个外网条件 skip/)
  assert.match(content, /下一阶段只收口上述 10 项“已实现待收口”，不启动 6 项“未实现”的新功能开发/)
