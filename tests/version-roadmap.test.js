...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*96%（24\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*96%（24\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 29, 'five reusable foundations plus twenty-four completed features')
  assert.equal(checkboxCount(content, false), 1)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 0)
  assert.equal((content.match(/（未实现）/g) || []).length, 1)
  assert.match(content, /#### C\. 按成果自动编排工作流（5\/5 闭环，5\/5 有代码）/)
  assert.match(content, /#### D\. 自然语言影音编辑 Agent v1（5\/5 闭环，5\/5 有代码）/)
  assert.match(content, /#### E\. 后台完成与主动交付（4\/5 闭环，4\/5 有代码）/)
  assert.match(content, /#### A\. 项目胶囊与持续记忆（5\/5 闭环，5\/5 有代码）/)
  assert.match(content, /#### B\. 任意文件与链接的统一理解（5\/5 闭环，5\/5 有代码）/)
  assert.match(content, /审计基线：`c155f75`/)
  assert.match(content, /相对 `master` 有 47 个提交、112 个文件发生变化、14563 行新增/)
  assert.match(content, /134 个自动测试文件、42 个安装态 smoke 脚本/)
  assert.match(content, /781 tests、781 pass、0 fail、0 skip/)
  assert.match(content, /下一阶段没有“待收口”尾巴/)
  assert.match(content, /余下 1 项保持“未实现”/)
