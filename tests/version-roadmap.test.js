...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*84%（21\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*84%（21\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 26, 'five reusable foundations plus twenty-one completed features')
  assert.equal(checkboxCount(content, false), 4)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 0)
  assert.equal((content.match(/（未实现）/g) || []).length, 4)
  ...
  assert.match(content, /#### B\. 任意文件与链接的统一理解（4\/5 闭环，4\/5 有代码）/)
  assert.match(content, /审计基线：`dfe501d`/)
  assert.match(content, /相对 `master` 有 39 个提交、97 个文件发生变化、12929 行新增/)
  assert.match(content, /128 个自动测试文件、36 个安装态 smoke 脚本/)
  assert.match(content, /757 tests、755 pass、0 fail、2 个外网条件 skip/)
  assert.match(content, /下一阶段没有“待收口”尾巴/)
  assert.match(content, /余下 4 项保持“未实现”/)
