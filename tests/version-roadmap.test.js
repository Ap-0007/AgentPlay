...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*24%（6\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*76%（19\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 11, 'five reusable foundations plus six completed features')
  assert.equal(checkboxCount(content, false), 19)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 13)
  assert.equal((content.match(/（未实现）/g) || []).length, 6)
  assert.match(content, /#### C\. 按成果自动编排工作流（1\/5 闭环，4\/5 有代码）/)
  assert.match(content, /#### D\. 自然语言影音编辑 Agent v1（4\/5 闭环，5\/5 有代码）/)
  ...
  assert.match(content, /下一阶段只收口上述 13 项“已实现待收口”，不启动 6 项“未实现”的新功能开发/)
