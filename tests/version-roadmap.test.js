...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*92%（23\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*92%（23\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 28, 'five reusable foundations plus twenty-three completed features')
  assert.equal(checkboxCount(content, false), 2)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 0)
  assert.equal((content.match(/（未实现）/g) || []).length, 2)
  ...
  assert.match(content, /审计基线：`e529c79`/)
  assert.match(content, /相对 `master` 有 44 个提交、106 个文件发生变化、14082 行新增/)
  assert.match(content, /130 个自动测试文件、38 个安装态 smoke 脚本/)
  assert.match(content, /773 tests、770 pass、0 fail、3 个外网条件 skip/)
  assert.match(content, /下一阶段没有“待收口”尾巴/)
  assert.match(content, /余下 2 项保持“未实现”/)
