...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*44%（11\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*76%（19\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 16, 'five reusable foundations plus eleven completed features')
  assert.equal(checkboxCount(content, false), 14)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 8)
  ...
  assert.match(content, /审计基线：`c9c78fd`/)
  assert.match(content, /相对 `master` 有 29 个提交、84 个文件发生变化、11131 行新增/)
  assert.match(content, /124 个自动测试文件、28 个安装态 smoke 脚本/)
  assert.match(content, /737 tests、735 pass、0 fail、2 个外网条件 skip/)
  assert.match(content, /下一阶段只收口上述 8 项“已实现待收口”，不启动 6 项“未实现”的新功能开发/)
