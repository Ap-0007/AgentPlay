...
test('0.9.0 reports strict closure separately from implementation coverage against the fixed denominator', () => {
  ...
  assert.match(content, /严格交付闭环：\*\*60%（15\/25）\*\*/)
  assert.match(content, /实质代码覆盖：\*\*76%（19\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 20, 'five reusable foundations plus fifteen completed features')
  assert.equal(checkboxCount(content, false), 10)
  assert.equal((content.match(/（已实现待收口）/g) || []).length, 4)
  ...
  assert.match(content, /#### A\. 项目胶囊与持续记忆（3\/5 闭环，3\/5 有代码）/)
  assert.match(content, /审计基线：`ebf6f99`/)
  assert.match(content, /相对 `master` 有 33 个提交、90 个文件发生变化、12127 行新增/)
  assert.match(content, /126 个自动测试文件、29 个安装态 smoke 脚本/)
  assert.match(content, /746 tests、746 pass、0 fail、0 skip/)
  assert.match(content, /下一阶段只收口上述 4 项“已实现待收口”，不启动 6 项“未实现”的新功能开发/)
