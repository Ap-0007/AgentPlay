...
test('0.9.1 records completed semantic and professional visual groups without overstating later work', () => {
  ...
  assert.match(content, /功能交付完成率：\*\*100%（25\/25）\*\*/)
  assert.equal(checkboxCount(content, true), 25)
  assert.equal(checkboxCount(content, false), 0)
  ...
  assert.match(content, /#### E\. AI 素材、个人风格与批处理（5\/5）/)
  ...
  assert.match(content, /第三十二个纵向切片（E4闭环并正式勾选）/)
  assert.match(content, /shared-media-edit-governance-v1/)
  assert.match(content, /20类持久任务/)
  assert.match(content, /hiddenExecutor=false/)
  assert.match(content, /toolCalls=1\/1/)
  assert.match(content, /1008 tests、1008 pass、0 fail、0 skip/)
  assert.match(content, /E组更新为4\/5，0\.9\.1更新为24\/25=96%/)
  assert.match(content, /第三十三个纵向切片（E5闭环并正式勾选）/)
  assert.match(content, /agentplay\.professional-corpus-e5/)
  assert.match(content, /20\/20质量100/)
  assert.match(content, /INVALID_SAMPLE_ASPECT_RATIO/)
  assert.match(content, /P50 5506\.9ms\/样本、P95 36077\.2ms\/样本/)
  assert.match(content, /1011 tests、1011 pass、0 fail、0 skip/)
  assert.match(content, /E组5\/5、0\.9\.1固定分母25\/25=100%闭环/)
})
