...
test('renderer presents remove-segment as a recoverable edit with every timeline mapping', () => {
  ...
  const types = read('src/types/global.d.ts')
  assert.match(tasks, /\['media\.trim', 'media\.remove-segment', 'media\.concat-segments', 'media\.add-music'\]\.includes\(decision\.kind\)/)
  ...
  assert.match(types, /kind: 'media\.trim' \| 'media\.remove-segment' \| 'media\.concat-segments' \| 'media\.add-music'/)
})
