...
test('main process owns planning, frozen execution, recovery and cancellation for exact trim jobs', () => {
  ...
  assert.match(main, /const taskType = decision\.kind === 'media\.concat-segments'/)
...
test('renderer presents remove-segment as a recoverable edit with every timeline mapping', () => {
  ...
  const types = read('src/types/global.d.ts')
  assert.match(tasks, /\['media\.trim', 'media\.remove-segment', 'media\.concat-segments'\]\.includes\(decision\.kind\)/)
  assert.match(tasks, /operation = decision\.kind === 'media\.remove-segment'/)
  assert.match(tasks, /timelineReceipt \|\| \[\]\)\.map/)
  assert.match(tasks, /start_remove_video_segment/)
  assert.match(runtime, /runtimeTask\.type === 'media\.edit-trim' \|\| runtimeTask\.type === 'media\.edit-remove' \|\| runtimeTask\.type === 'media\.edit-concat'/)
  assert.match(types, /kind: 'media\.trim' \| 'media\.remove-segment' \| 'media\.concat-segments'/)
})
...
test('renderer and preload contracts present concat and reorder as one recoverable editing action', () => {
  ...
  assert.match(runtime, /runtimeTask\.type === 'media\.edit-concat'/)
