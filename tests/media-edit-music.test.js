  ...
  try {
    ...
    const frames = {
      ...
      probeDuration: async (file) => {
        const r = spawnSync(FFMPEG.replace('ffmpeg.exe', 'ffprobe.exe'), ['-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', file], { timeout: 30000 })
        return Number(String(r.stdout).trim())
      },
      probeHasAudio: async (file) => {
        const r = spawnSync(FFMPEG.replace('ffmpeg.exe', 'ffprobe.exe'), ['-v', 'error', '-select_streams', 'a:0', '-show_entries', 'stream=codec_type', '-of', 'csv=p=0', file], { timeout: 30000 })
        return String(r.stdout).includes('audio')
      },
      run: async (args) => {
        const r = spawnSync(FFMPEG, args, { timeout: 120000 })
        if (r.status !== 0) throw new Error(String(r.stderr).slice(0, 300))
      }
    }
