...
test('notification text reflects real completed, approval and failure states without exposing full paths', () => {
  ...
  const posixCompleted = notificationText(makeTask('completed', { result: { outputs: ['/home/user/private/合同处理版.pdf'] } }))
  assert.equal(posixCompleted.body, '文档任务：已生成 合同处理版.pdf')
  assert.doesNotMatch(posixCompleted.body, /\/home\/user\/private/)
