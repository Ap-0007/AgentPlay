...
try {
  ...
  const result = await evaluate(`(async () => {
    ...
    if (task.state !== 'completed' || task.quality?.score !== 100 || task.result?.outputs?.length !== 2 || task.result?.repairReceipt?.stabilization?.verdict !== 'improved' || task.result?.repairReceipt?.color?.verdict !== 'improved' || task.result?.repairReceipt?.sourceUnchanged !== true || task.result?.visualQc?.passed !== true || task.result.visualQc.artifacts?.length !== 2) throw new Error(task.error || '安装态画面修复质量门失败')
  ...
  process.stdout.write(`${JSON.stringify({ receiptPath, quality: receipt.result.quality.score, visualQc: receipt.result.visualQc.passed, qcArtifacts: receipt.result.visualQc.artifacts.length, stabilization: receipt.result.repairReceipt.stabilization, color: { beforeDistance: receipt.result.repairReceipt.color.beforeDistance, afterDistance: receipt.result.repairReceipt.color.afterDistance }, repairedDimensions: receipt.result.repairedDimensions, comparisonDimensions: receipt.result.comparisonDimensions, findings: receipt.result.repairReceipt.lowQualityFindings.length, sourceUnchanged: true })}\n`)
