...
const modelPort = await freePort(); const modelPrompts = []
const modelServer = http.createServer((request, response) => {
  ...
  let body = ''; request.setEncoding('utf8'); request.on('data', (chunk) => { body += chunk }); request.on('end', () => {
    const parsed = JSON.parse(body); modelPrompts.push(parsed.messages?.map((item) => item.content).join('\n') || '')
    response.writeHead(200, { 'Content-Type': 'application/json' }); response.end(JSON.stringify({ choices: [{ message: { content: JSON.stringify(modelPayload) } }], usage: { prompt_tokens: 320, completion_tokens: 180 } }))
  })
})
await new Promise((resolve, reject) => { modelServer.once('error', reject); modelServer.listen(modelPort, '127.0.0.1', resolve) })
const modelRecord = { providerId: 'vllm', model: 'mock-semantic-reviewer', baseUrl: `http://127.0.0.1:${modelPort}/v1`, encryptedApiKey: '', updatedAt: new Date().toISOString() }
fs.writeFileSync(path.join(profileDir, 'model-config.json'), JSON.stringify({ schemaVersion: 3, roles: { chat: modelRecord }, profiles: { chat: [modelRecord] }, updatedAt: new Date().toISOString() }, null, 2), 'utf8')
...
try {
  ...
  const pageResult = await evaluate(`(async () => {
    ...
    await waitFor(() => document.body.innerText.includes('语义近似重复') && document.body.innerText.includes('疑似跑题') && document.body.innerText.includes('请回复“确认执行”或“取消”'), '模型语义确认方案可见', 120000)
    ...
    if (task.state !== 'completed' || task.quality?.passed !== true || task.result?.semanticCut?.strategy !== 'model-semantic-review-v1' || task.result?.semanticCut?.removed?.length !== 2) throw new Error(task.error || '模型语义剪辑质量门失败')
  ...
  if (modelPrompts.length !== 1 || !modelPrompts[0].includes('[2][1.80-3.00]') || !modelPrompts[0].includes('只能引用以上字幕序号')) throw new Error('安装态模型没有收到唯一且可核验的字幕证据请求')
  const outputPath = pageResult.task.result.outputPath
  const receipt = { acceptedAt: new Date().toISOString(), executable, source: { path: sourcePath, durationSeconds: probeDuration(sourcePath), sha256: sourceHash, unchanged: sha256(sourcePath) === sourceHash }, modelCalls: modelPrompts.length, decision: pageResult.task.spec.decision, result: { outputPath, durationSeconds: probeDuration(outputPath), quality: pageResult.task.quality }, ui: { confirmationVisible: pageResult.body.includes('请回复“确认执行”或“取消”'), previewDuration: pageResult.preview.duration, undoDuration: pageResult.undo.duration } }
  ...
  process.stdout.write(`${JSON.stringify({ receiptPath, modelCalls: receipt.modelCalls, removed: receipt.decision.semanticCut.removed.length, model: receipt.decision.semanticCut.modelEvidence.model, quality: receipt.result.quality.score, outputDuration: receipt.result.durationSeconds, undoDuration: receipt.ui.undoDuration, sourceUnchanged: true })}\n`)
