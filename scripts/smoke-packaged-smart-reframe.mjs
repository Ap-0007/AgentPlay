...
try {
  ...
  const result = await evaluate(`(async () => {
    ...
    if (first.state !== 'completed' || first.quality?.score !== 100 || first.result?.outputs?.length !== 3 || first.result?.trackingReceipt?.subject?.description !== '红色人物框') throw new Error(first.error || '第一轮智能构图质量门失败')
    ...
    if (second.state !== 'completed' || second.quality?.score !== 100 || second.result?.outputs?.length !== 3 || second.result?.trackingReceipt?.subject?.description !== '蓝色人物框' || second.spec?.decision?.reframe?.correctionOf?.subject !== '红色人物框' || second.spec?.decision?.source?.path !== ${JSON.stringify(sourcePath)}) throw new Error(second.error || '修正对象没有从原片重新生成')
  ...
  process.stdout.write(`${JSON.stringify({ receiptPath, firstQuality: receipt.first.quality.score, correctionQuality: receipt.correction.quality.score, firstSubject: receipt.first.trackingReceipt.subject.description, correctionSubject: receipt.correction.trackingReceipt.subject.description, firstCenter, correctionCenter, outputs: receipt.first.outputs.length + receipt.correction.outputs.length, modelRequests: modelRequests.length, sourceUnchanged: true })}\n`)
